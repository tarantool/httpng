...
- `add_routes()`: add route handlers to the running server.
Accepts two parameters: the first one is a table or array of tables
(see "Route handlers" below), the second one (optional)
is a string specifying insertion point - `path` of the route handler
before which new entries would be inserted
("after all existing route handlers" is the default).
...
- `remove_routes()`: Removes route handlers from the running server,
accepts a single parameter - string or array of strings specifying
...
- `handler`: Function, default HTTP(S) request handler.
You can use `routes` for more flexibility and native file/folder hosting.
...
- `routes`: Array of tables describing request "route handlers" (see below).
`handler` is used if no route handler match was found.
Note that specifying this field when reconfiguring running server
would replace all existing routes, you can use `add_routes()`/`remove_routes()`
...
You can add/remove/replace `handler` or modify `routes`
(using `add_routes()`/`remove_routes()`),
increase or decrease `threads`, change `thread_termination_timeout`.
...
`routes` in cfg table specifies an array with tables specified above.
