...
- `router`: router object (Lua table technically)
...
You can modify router object by calling
...
Paths with placeholders are sorted by "longest prefix first"
("/" is the boundary). If your `path`s do not clash and use the same `method`
and `host` specifications you do not need to bother with entries order
and the performance would be maximal.
TODO: API to query "what degrades performance".
